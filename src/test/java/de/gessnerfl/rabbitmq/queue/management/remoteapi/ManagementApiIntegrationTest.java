...
public class ManagementApiIntegrationTest extends AbstractIntegrationTest {
  private final static Logger LOGGER = LoggerFactory.getLogger(ManagementApiIntegrationTest.class);
    ...
    for(Queue q : queues){
      if(q.getName().equals(queueName)){
        ...
        if(isDeadLettered){
          ...
          assertEquals(DEAD_LETTERED_QUEUE_NAME, q.getArguments().get(Queue.DEAD_LETTER_ROUTINGKEY_ARGUMENT));
        }
      }
    }
    assertTrue(found);
